...
pub contract YDYNFT: NonFungibleToken {
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver {
        ...
        init(
            ...
            YDYNFT.totalSupply = YDYNFT.totalSupply + 1
            self.id = YDYNFT.totalSupply
            ...
            self.endurance = UFix64(YDYNFT.calculateAttribute(rarity))
            self.lastEnduranceBoost = getCurrentBlock().timestamp
            self.efficiency = UFix64(YDYNFT.calculateAttribute(rarity))
            self.lastEfficiencyBoost = getCurrentBlock().timestamp
            self.luck = UFix64(YDYNFT.calculateAttribute(rarity))
            self.lastLuckBoost = getCurrentBlock().timestamp

            self.rarity = YDYNFT.rarityToString(rarity)
            ...
            switch view {
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(
                        storagePath: YDYNFT.CollectionStoragePath,
                        publicPath: YDYNFT.CollectionPublicPath,
                        providerPath: /private/ydyNFTCollection,
                        publicCollection: Type<&YDYNFT.Collection{YDYNFT.YDYNFTCollectionPublic}>(),
                        publicLinkedType: Type<&YDYNFT.Collection{YDYNFT.YDYNFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(),
                        providerLinkedType: Type<&YDYNFT.Collection{YDYNFT.YDYNFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Provider,MetadataViews.ResolverCollection}>(),
                        createEmptyCollectionFunction: (fun (): @NonFungibleToken.Collection {
                            return <-YDYNFT.createEmptyCollection()
                        })
                    )
                ...
                case Type<MetadataViews.Royalties>():
                    return MetadataViews.Royalties(
                        [MetadataViews.Royalty(recepient: getAccount(YDYNFT.account.address).getCapability<&{FungibleToken.Receiver}>(/public/flowTokenReceiver), cut: 0.075, description: "This is the royalty receiver for YDY NFTs")]
                    )  
            }
            return nil
        }
    ...
    pub resource interface YDYNFTCollectionPublic {
        ...
        pub fun borrowYDYNFT(id: UInt64): &YDYNFT.NFT? {
            post {
    ...
    pub fun buy(collectionCapability: Capability<&Collection{YDYNFT.YDYNFTCollectionPublic}>, payment: @FlowToken.Vault, quantity: UInt64) {
        pre {
            YDYNFT.isMintingEnabled == true: "Minting is not enabled."
            payment.balance == YDYNFT.price * UFix64(quantity): "Payment does not match the price."
		}
        ...
        let ydyCollection = self.account.getCapability(YDYNFT.CollectionPublicPath)
                                .borrow<&Collection{YDYNFT.YDYNFTCollectionPublic, YDYNFT.YDYNFTCollectionPrivate}>()
        ...
        if (availableNFTs.length > 0) {
            ...
            let token <- ydyCollection.withdraw(withdrawID: id) as! @YDYNFT.NFT
    ...
    pub resource Admin {
        ...
        pub fun mintNFT(
            ...
            let accountOwnerCollection = YDYNFT.account.borrow<&Collection{NonFungibleToken.CollectionPublic}>(from: YDYNFT.CollectionStoragePath)!
            accountOwnerCollection.deposit(token: <-create YDYNFT.NFT(thumbnailCID: thumbnailCID, background: background, body: body, mouth: mouth, eyes: eyes, pants: pants, rarity: rarity, version: version, versionLaunchDate: versionLaunchDate))
        }

        pub fun transferNFT(collectionCapability: Capability<&Collection{YDYNFT.YDYNFTCollectionPublic}>): UInt64 {
            let ydyCollection = YDYNFT.account.getCapability(YDYNFT.CollectionPublicPath)
                                    .borrow<&Collection{YDYNFT.YDYNFTCollectionPublic, YDYNFT.YDYNFTCollectionPrivate}>()
                                    ?? panic("Can't get YDY's collection.")
        ...
        pub fun levelUp(id: UInt64, recipientCollectionCapability: Capability<&Collection{YDYNFT.YDYNFTCollectionPublic}>): &YDYNFT.NFT? {
            post {
        ...
        pub fun repair(points: UFix64, id: UInt64, recipientCollectionCapability: Capability<&Collection{YDYNFT.YDYNFTCollectionPublic}>): &YDYNFT.NFT? {
            let receiver = recipientCollectionCapability.borrow() ?? panic("Cannot borrow")
            let nft = receiver.borrowYDYNFT(id: id) ?? panic("No NFT with this ID exists for user")
        ...
        pub fun reduceStamina(points: UFix64, id: UInt64, recipientCollectionCapability: Capability<&Collection{YDYNFT.YDYNFTCollectionPublic}>): &YDYNFT.NFT? {
            post {
        ...
        pub fun boostEndurance(points: UFix64, id: UInt64, recipientCollectionCapability: Capability<&Collection{YDYNFT.YDYNFTCollectionPublic}>): &YDYNFT.NFT? {
            let receiver = recipientCollectionCapability.borrow() ?? panic("Cannot borrow")
            let nft = receiver.borrowYDYNFT(id: id) ?? panic("No NFT with this ID exists for user")
        ...
        pub fun boostEfficiency(points: UFix64, id: UInt64, recipientCollectionCapability: Capability<&Collection{YDYNFT.YDYNFTCollectionPublic}>): &YDYNFT.NFT? {
            let receiver = recipientCollectionCapability.borrow() ?? panic("Cannot borrow")
            let nft = receiver.borrowYDYNFT(id: id) ?? panic("No NFT with this ID exists for user")
        ...
        pub fun boostLuck(points: UFix64, id: UInt64, recipientCollectionCapability: Capability<&Collection{YDYNFT.YDYNFTCollectionPublic}>): &YDYNFT.NFT? {
            let receiver = recipientCollectionCapability.borrow() ?? panic("Cannot borrow")
            let nft = receiver.borrowYDYNFT(id: id) ?? panic("No NFT with this ID exists for user")
        ...
        pub fun changeIsMintingEnabled(isMinting: Bool) {
            YDYNFT.isMintingEnabled = isMinting
        }
    }
